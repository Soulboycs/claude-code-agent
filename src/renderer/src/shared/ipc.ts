...
export interface DesktopApi {
  ...
  /** off-thread DEFLATE: the main process runs the engine saveDocx (zip splice +
   *  compression) and returns the bytes, so saving a large document never
   *  freezes the renderer; falls back to the local engine call when absent */
  zipSave(payload: {
    parsed: unknown
    finalBlocks: unknown[]
    options?: unknown
