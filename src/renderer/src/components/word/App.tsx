...
import { blockRangePositions, buildDocumentContext, type SelectionScope } from './ai/protocol'
...
import { AiAskPopover, SelectionContext } from './components/AiAskPopover'
...
import { resolvePageFootnotes, syncNoteRefSupDisplay } from './footnote-pagination'
...
import { TableSortModal } from './components/TableSortModal'
import { buildCompareFinalBlocks } from './editor/compare-docx'
import { findTableAtSelection } from './editor/table-sort'
import { zipDocxBytes } from './zip-save'
import {
  EditorContextMenu,
  FontDialog,
  ParagraphDialog,
...
import { stylerefText, type StylerefEntry } from './editor/styleref'
import type { LayoutSnapshot } from './editor/layout-history'
import { LayoutUndoExtension } from './editor/layout-undo-extension'
...
import { getLastOpenedWordFile } from './persistence'
...
export function App() {
  ...
  /** heading -> page map of the last pagination pass (STYLEREF edge-strip resolution) */
  const stylerefHeadingsRef = useRef<StylerefEntry[]>([])
  const edgeStyleRefResolver = useCallback(
    (styleName: string, page: number) => stylerefText(stylerefHeadingsRef.current, styleName, page),
    [],
  )
  ...
  // ---- layout undo: margins/orientation/section settings join the editor's
  // own history via a doc-attribute mirror (see layout-undo-extension.ts).
  // The ref is the source for building the NEXT snapshot: AI review actions
  // can run several layout edits between renders, so it updates synchronously
  // on every apply, not only during render.
  const layoutStateRef = useRef<LayoutSnapshot>({
    section: null,
    sections: [],
    sectionDirty: false,
    sectionsDirty: [],
  ...
  const restoreLayoutSnapshot = useCallback((snap: LayoutSnapshot) => {
    setSection(snap.section)
    setSections(snap.sections)
    setSectionDirty(snap.sectionDirty)
    setSectionsDirty(snap.sectionsDirty)
    setPgNumEdit(snap.pgNumEdit)
    setPgNumDirtySections(snap.pgNumDirtySections)
    setTitlePg(snap.titlePg)
    setTitlePgDirty(snap.titlePgDirty)
  }, [])
  /** apply a layout state through the PM history mirror (undoable as one step) */
  const applyLayoutState = useCallback(
    (snap: LayoutSnapshot) => {
      layoutStateRef.current = snap
      restoreLayoutSnapshot(snap)
      const ed = editorRef.current
      if (ed) {
        ed.view.dispatch(
          ed.state.tr.setMeta('layoutUndoMirror', true).setDocAttribute('layoutUndo', snap),
        )
      }
    },
  ...
  /** Whether a header/footer area is currently being edited (double-clicked) – shows the contextual HF ribbon tab */
  const [activeHfEditing, setActiveHfEditing] = useState(false)
  ...
  useEffect(() => {
    const handleToggle = () => {
      setRevisionDisplay((prev) => (prev === 'simple' ? 'all' : 'simple'))
    }
    window.addEventListener('nexus-word-toggle-rev-display', handleToggle)
    return () => window.removeEventListener('nexus-word-toggle-rev-display', handleToggle)
  }, [])
  ...
  const editor = useEditor({
    extensions: [
      ...editorExtensions,
      // layout mirror: one history entry per layout edit, restored on undo/redo
      LayoutUndoExtension.configure({
        onRestore: (snap: unknown) => restoreLayoutSnapshot(snap as LayoutSnapshot),
      }),
    ],
    ...
    onFocus: () => {
      ...
      setInactiveSelectionShown(editorRef.current, false)
    },
    onBlur: () => {
      const ed = editorRef.current
    ...
    onUpdate: ({ transaction }) => {
      // layout mirror transactions (and their history undo/redo) only touch
      // the layoutUndo doc attribute: layout dirty flags carry the change,
      // the document itself must not turn dirty
      if (transaction.getMeta('layoutUndoMirror')) return
      if (
        transaction.getMeta('history$') &&
        transaction.steps.every((step) => step instanceof AttrStep)
      )
        return
  ...
  useEffect(() => {
    ...
    void bootPendingRef.current
      .then(async ([pending, , aiContent]) => {
        ...
        let outcome = pending ? await loadFile(pending) : 'canceled'
        if (outcome === 'canceled') {
          // Check for persisted last opened file to seamlessly restore upon page switch or reload
          const lastFile = getLastOpenedWordFile()
          if (lastFile) {
            try {
              const res = await window.desktop.openDocxPath(lastFile)
              if (res && !('error' in res)) {
                outcome = await loadFile(res)
              }
            } catch (err) {
              console.warn('Failed to restore last opened word file:', err)
            }
          }
        }
  ...
  /** Compare panel: export the merged revision document (w:ins / w:del) */
  const exportCompareDocx = useCallback(async () => {
    if (!doc || !compareResult?.otherBlocks) return
    try {
      const { finalBlocks } = buildCompareFinalBlocks(
        doc.parsed.blocks,
        compareResult.otherBlocks,
      )
      const bytes = await zipDocxBytes(doc.parsed, finalBlocks, {})
      const name = `${doc.fileName.replace(/\.docx$/i, '')}-compare.docx`
      const buffer = bytes.buffer.slice(
        bytes.byteOffset,
        bytes.byteOffset + bytes.byteLength,
      ) as ArrayBuffer
      const res = await window.desktop.saveDocxNew(name, buffer)
      setStatus(res.ok && res.path ? `${t('appCompareExport')}: ${res.path}` : t('appCompareFailed', { error: res.error ?? '' }))
    } catch (err) {
      setStatus(t('appCompareFailed', { error: String(err) }))
    }
  }, [doc, compareResult, t])
  ...
  const revisionAuthors = useMemo(
    () => (editor && doc ? distinctRevisionAuthors(editor.state.doc) : []),
    [editor, doc, revisionCount],
  )
  const revisionFilterCss = useMemo(() => {
  ...
  const toggleRevAuthor = useCallback(
    (author: string) => {
      setRevAuthorFilter((prev) => {
        // from "all" (null): unchecking one author keeps the rest checked
        const authors = new Set(prev ?? revisionAuthors)
        if (authors.has(author)) authors.delete(author)
        else authors.add(author)
        // everything checked again = no filter
        return authors.size === revisionAuthors.length && revisionAuthors.length > 0 ? null : authors
      })
    },
    [revisionAuthors],
  )
  const filteredApply = useCallback(
    (mode: 'accept' | 'reject') => {
      if (!editor) return
      // display modes other than 'all' ignore the filter (contract D4)
      const active =
        revisionDisplay === 'all' && revAuthorFilter !== null ? revAuthorFilter : null
      applyRevisions(editor, revisionsOfAuthors(editor.state.doc, active), mode)
    },
    [editor, revAuthorFilter, revisionDisplay],
  )
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver(() => {
      // While sidebar is being dragged, defer fit to avoid full React re-render & pagination on every frame
      const isDraggingSidebar =
      ...
      if (isDraggingSidebar) {
        ...
        resizeTimer = window.setTimeout(applyResizeFit, 120)
        return
      }

      // Normal window resize: light debounce to coalesce rapid resize events
      if (resizeTimer) window.clearTimeout(resizeTimer)
      resizeTimer = window.setTimeout(applyResizeFit, 40)
    })
    ro.observe(el)
  ...
  const pageFootnotesOf = useCallback(
    (blocks: BlockBox[], slices: PageSlice[]): PageNoteItem[][] => {
      if (!doc || footnotes.length === 0 || !section) return slices.map(() => [])
      return resolvePageFootnotes(blocks, slices, footnotes, {
        section,
        sections,
        docFootnoteProps: doc.parsed.footnoteProps,
        blockByDocxIndex,
        noteRenderInfoOf,
        fallbackNoteNo: (id, idx) => noteNo('footnote', id, idx),
      })
    },
    ...
    const remeasurePass = () => {
      ...
      if (editor) {
        ...
        if (viewMode === 'print' && !readMode) {
          ...
          syncNoteRefSupDisplay(pm, pageNotes)
          ...
          // STYLEREF headers/footers resolve against the same slices the gap
          // strips render into: every styled paragraph -> block top -> 1-based
          // page (the same mapping the TOC page backfill uses further below)
          const stylerefEntries: StylerefEntry[] = []
          {
            const topByIdx = new Map<number, number>()
            for (const b of blocks) if (b.docxIndex != null) topByIdx.set(b.docxIndex, b.top)
            const styleNameOf = (styleId: unknown) => {
              if (typeof styleId !== 'string' || !styleId) return null
              const name = doc?.parsed.styles?.get(styleId)?.name
              return name ?? null
            }
            editor.state.doc.forEach((node) => {
              const name = styleNameOf((node.attrs as Record<string, unknown>)?.styleId)
              if (!name || !node.textContent.trim()) return
              const di = (node.attrs as Record<string, unknown>)?.docxIndex as
                | number
                | null
                | undefined
              const top = di == null ? undefined : topByIdx.get(di)
              stylerefEntries.push({
                styleName: name,
                text: node.textContent,
                page: top != null ? pageAt(slices, top + 1) : undefined,
              })
            })
          }
          const resolveGapStyleRef = (styleName: string, page: number) =>
            stylerefText(stylerefEntries, styleName, page)
          // the resting edge strips read this ref: re-measuring during render
          // would thrash, so they resolve against the last pagination pass
          stylerefHeadingsRef.current = stylerefEntries
          slices.slice(1).forEach((slice, k) => {
            ...
            const prevSecInfo = secList?.[slices[k].section]
            const nextSecInfo = secList?.[slice.section]
            const prevSec = prevSecInfo?.settings ?? section
            const nextSec = nextSecInfo?.settings ?? section
            ...
            if (hfHasVisibleContent(gapFooter.value, gapFooter.images)) {
              ...
              const el = makeGapHfEl({
                ...
                // the footer strip belongs to the page above the gap (1-based)
                pageNoNum: k + 1,
                resolveStyleRef: resolveGapStyleRef,
                pageTotal: visiblePages,
                sectionIndex: prevSecInfo ? sections.indexOf(prevSecInfo) : 0,
                geom: { ...hfStripGeom(prevSec), stripLeft: gapStripLeft(prevSec, box) },
                readOnly: isProtected || readMode,
                onCommit: (next) => {
                  if (multiHf && sections.length > 1 && prevSecInfo) {
                    setSectionHfEdits((m) => ({ ...m, [`${prevSecInfo.lastBlockIndex}:footer`]: next }))
                    setFooterDirty(true)
                  } else {
                    commitHf('footer', next)
                  }
                },
              })
              el.style.top = 'auto'
            ...
            if (hfHasVisibleContent(gapHeader.value, gapHeader.images)) {
              ...
              const el = makeGapHfEl({
                ...
                resolveStyleRef: resolveGapStyleRef,
                pageTotal: visiblePages,
                sectionIndex: nextSecInfo ? sections.indexOf(nextSecInfo) : 0,
                geom: {
                  ...hfStripGeom(nextSec),
                  headerStripTop,
                ...
                readOnly: isProtected || readMode,
                onCommit: (next) => {
                  if (multiHf && sections.length > 1 && nextSecInfo) {
                    setSectionHfEdits((m) => ({ ...m, [`${nextSecInfo.lastBlockIndex}:header`]: next }))
                    setHeaderDirty(true)
                  } else {
                    commitHf('header', next)
                  }
                },
              })
  ...
  useEffect(() => {
    ;(window as unknown as Record<string, unknown>).__aidocs = {
      ...
      getFilePath: () => fileCtxRef.current.doc?.filePath,
      filePath: doc?.filePath,
      getDocumentContext: (scope?: SelectionScope) =>
        editor ? buildDocumentContext(editor, scope) : '',
      getBlockRange: (from: number, to: number) =>
        editor ? getBlockRange(editor, from, to) : null
    }
  }, [editor, openRecent, save, status, exportPdf, exportHtml, doc?.filePath])

  // Listen for external file changes and explicit open-file requests from Agent
  useEffect(() => {
    const handleOpenFile = (e: Event) => {
      const detail = (e as CustomEvent<{ path: string }>).detail
      if (detail?.path) {
        // 活动文件守卫：对比 fileCtxRef.current.doc?.filePath 与目标路径，
        // 若目标文档已处于打开激活状态，坚决不调用 openRecent(detail.path)，彻底消灭工具执行中的白屏和跳顶（scrollTop 归零）。
        const currentPath = fileCtxRef.current.doc?.filePath
        if (currentPath) {
        ...
        void openRecent(detail.path)
      }
    }
    window.addEventListener('nexus-word-open-file', handleOpenFile)

    const unsubscribeChanged = window.desktop?.onWordFileChanged?.((detail: { filePath: string }) => {
      ...
      if (currentPath) {
        ...
        if (normCurrent === normTarget) {
          // Document is already active and held live in editor memory; NEVER reload/reset viewport!
          return
        }
      }
      void openRecent(detail.filePath)
    })
  ...
  // Listen for block navigation requests (e.g. from docnav://block/:index links)
  useEffect(() => {
    if (!editor) return
    const handleNavigateBlock = (e: Event) => {
      const detail = (e as CustomEvent<{ blockIndex: number }>).detail
      if (detail && typeof detail.blockIndex === 'number') {
        navigateToBlock(editor, detail.blockIndex)
        try {
          const clamped = Math.min(Math.max(detail.blockIndex, 0), editor.state.doc.childCount - 1)
          const { from } = blockRangePositions(editor, clamped, clamped)
          let el: HTMLElement | null = null
          const nodeDom = editor.view.nodeDOM(from)
          if (nodeDom instanceof HTMLElement) {
            el = nodeDom
          } else {
            const atPos = editor.view.domAtPos(Math.min(from + 1, editor.state.doc.content.size))
            const node = atPos?.node
            el = (node instanceof HTMLElement ? node : node?.parentElement) ?? null
          }
          if (el) {
            el.classList.add('ai-nav-target')
            setTimeout(() => {
              el?.classList.remove('ai-nav-target')
            }, 2500)
          }
        } catch (err) {
          console.warn('[word] Failed to pulse target block element:', err)
        }
      }
    }
    window.addEventListener('nexus-word-navigate-block', handleNavigateBlock)
    return () => {
      window.removeEventListener('nexus-word-navigate-block', handleNavigateBlock)
    }
  }, [editor])

  // Listen for header/footer editing state changes (double-click on HF area activates contextual ribbon tab)
  useEffect(() => {
    const handleHfEdit = (e: Event) => {
      const detail = (e as CustomEvent<{ active: boolean }>).detail
      if (detail && typeof detail.active === 'boolean') {
        setActiveHfEditing(detail.active)
      }
    }
    window.addEventListener('nexus-word-edit-hf', handleHfEdit)
    return () => {
      window.removeEventListener('nexus-word-edit-hf', handleHfEdit)
    }
  }, [])
  ...
  const getBlockRange = (
    ...
    from: number,
    to: number
  ): { startBlockIndex: number; endBlockIndex: number; blockCount: number } => {
    let startIndex = -1
    let endIndex = -1
    let index = 0
    ...
    const end = endIndex !== -1 ? endIndex : start
    return {
      startBlockIndex: start,
      endBlockIndex: end,
      blockCount: Math.max(1, end - start + 1)
    }
  }

  const askSendNow = (text: string, context?: SelectionContext): void => {
    ...
    if (context?.from !== undefined && editor) {
      ...
      const range = getBlockRange(editor, context.from, to)
      startBlockIndex = range.startBlockIndex
      endBlockIndex = range.endBlockIndex
      blockCount = range.blockCount

      // 在发送瞬间立即执行 editor.commands.setTextSelection(to) 折叠清除浏览器的深蓝文字遮罩，使得改动高亮立即显现！
      try {
        editor.commands.setTextSelection(to)
        setInactiveSelectionShown(editor, false)
      } catch (err) {
        console.warn('[word] Failed to collapse text selection:', err)
      }
    }
      ...
      try {
        const scope: SelectionScope | undefined =
          context?.from !== undefined
            ? {
                startIndex: startBlockIndex ?? 0,
                endIndex: endBlockIndex ?? 0,
                isRange: context.from !== (context.to ?? context.from),
                from: context.from,
                to: context.to ?? context.from
              }
            : undefined
        docContext = buildDocumentContext(editor, scope)
      } catch (err) {
        console.warn('[word] Failed to build document context:', err)
      }
    }
    ...
    window.dispatchEvent(
      new CustomEvent('nexus-word-ask-ai', {
        detail: {
          ...
          endBlockIndex,
          blockCount,
          filePath: doc?.filePath || 'Word-Document.docx',
          docContext
        }
      })
    )
  ...
  const aiPageSetupAccess = useMemo<AiPageSetupAccess>(
    () => ({
      ...
      set: (index, resolved) => {
        ...
        const sectionsNext = base.sections.map((s, i) => (i === index ? next : s))
        if (sec.pendingBreak) {
          // pending-break sectPr lives in the PM document itself; only the
          // section model mirrors through layout state
          applyLayoutState({ ...base, sections: sectionsNext })
          return null
        }
        const last = ctx.sections.length <= 1 || index === ctx.sections.length - 1
        applyLayoutState({
          ...base,
          sections: sectionsNext,
          section: last ? resolved.settings : base.section,
          sectionDirty: last ? true : base.sectionDirty,
          sectionsDirty: last
            ? base.sectionsDirty
            : base.sectionsDirty.includes(index)
              ? base.sectionsDirty
              : [...base.sectionsDirty, index],
          titlePg:
            last && resolved.titlePg !== undefined ? resolved.titlePg : base.titlePg,
          titlePgDirty:
            last && resolved.titlePg !== undefined ? true : base.titlePgDirty,
          pgNumEdit: last && resolved.pgNum ? resolved.pgNum : base.pgNumEdit,
          pgNumDirtySections: resolved.pgNum
            ? base.pgNumDirtySections.includes(index)
              ? base.pgNumDirtySections
              : [...base.pgNumDirtySections, index]
            : base.pgNumDirtySections,
        })
        return null
      },
  ...
  const ribbonActions = useStableCallbacks({
    ...
    onSection: (next: SectionSettings) => {
      // layout applies to the cursor's section; the final section's sectPr goes
      // through SaveOptions.section (also drives canvas geometry). Both paths
      // go through the history mirror: one Ctrl+Z entry per dialog apply.
      const base = layoutStateRef.current
      const last = base.sections.length <= 1 || activeSection === base.sections.length - 1
      applyLayoutState({
        ...base,
        sections: base.sections.map((s, i) =>
          i === activeSection ? { ...s, settings: next } : s,
        ),
        section: last ? next : base.section,
        sectionDirty: last ? true : base.sectionDirty,
        sectionsDirty: last
          ? base.sectionsDirty
          : base.sectionsDirty.includes(activeSection)
            ? base.sectionsDirty
            : [...base.sectionsDirty, activeSection],
      })
      if (!last) setStatus(t('appSectionSettingsApplied', { n: activeSection + 1 }))
    },
    ...
    onTableSort: () => {
      if (!editor) return
      if (!findTableAtSelection(editor)) {
        setStatus(t('appSortNoTable'))
        return
      }
    ...
    onAcceptRevision: (all: boolean) =>
      all && revAuthorFilter !== null && revisionDisplay === 'all'
        ? filteredApply('accept')
        : handleRevision('accept', all),
    onRejectRevision: (all: boolean) =>
      all && revAuthorFilter !== null && revisionDisplay === 'all'
        ? filteredApply('reject')
        : handleRevision('reject', all),
  ...
  return (
    ...
    >
      ...
      {doc && revisionFilterCss && <style>{revisionFilterCss}</style>}
      ...
      {colFlow && viewMode === 'print' && (
        ...
        <style>{`.editor-scroll .doc-page { column-count: ${colFlow.cols}; column-gap: ${colFlow.gapPx}px; column-fill: balance;${(canvasSection ?? section)?.colSep ? ' column-rule: 1pt solid var(--docs-paper-ink-soft, currentColor);' : ''} }
.editor-scroll .doc-page.measuring-columns { column-count: auto; width: ${colFlow.colWidthPx + twipsToPx(canvasSection?.marginLeft ?? section?.marginLeft ?? 0) + twipsToPx(canvasSection?.marginRight ?? section?.marginRight ?? 0)}px; }`}</style>
      ...
      <Ribbon
        ...
        revisionAuthorFilter={revAuthorFilter}
        onToggleRevisionAuthor={toggleRevAuthor}
        ...
        showGrid={showGrid}
        splitView={splitView}
        activeHfEditing={activeHfEditing}
        onCloseHf={() => {
          setActiveHfEditing(false)
          // blur the active HF edit surface so it commits and closes
          const el = document.querySelector('.page-hf-editing .page-hf-edit-surface') as HTMLElement | null
          el?.blur()
        }}
        {...ribbonActions}
      />
      ...
      <div className="app-main">
        <div className={`app-content${activeHfEditing ? ' doc-mode-hf-active' : ''}`}>
          <div className={workspaceClass}>
                ...
                {doc ? (
                  ...
                  >
                    {showRuler && section && (
                      <VRuler
                        section={section}
                        onVerticalMargins={({ marginTop, marginBottom }) => {
                    ...
                    <div
                      className={mixedPaper ? 'page-wrap paper-mixed' : 'page-wrap'}
                      onDoubleClick={(e) => {
                        if (activeHfEditing) {
                          const target = e.target as HTMLElement
                          if (!target.closest('.page-hf-editing') && !target.closest('.page-hf-section-tag')) {
                            e.stopPropagation()
                            e.preventDefault()
                            const el = document.querySelector(
                              '.page-hf-editing .page-hf-edit-surface',
                            ) as HTMLElement | null
                            el?.blur()
                            setActiveHfEditing(false)
                          }
                    ...
                    >
                      ...
                      {Boolean(
                        ...
                        <HeaderFooterArea
                          ...
                          pageNoNum={1}
                          resolveStyleRef={edgeStyleRefResolver}
            ...
            {doc && compareResult && (
              <ComparePanel
                ...
                onExport={() => void exportCompareDocx()}
      ...
      {showSortModal && editor && (
        <TableSortModal editor={editor} onClose={() => setShowSortModal(false)} />
      )}
      ...
      {doc && showPagePreview && section && (
        <PaginationPreview
          ...
          styleRefResolver={edgeStyleRefResolver}
          clearPageGaps={() => {
