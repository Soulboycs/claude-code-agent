...
export interface ImagePatch {
  ...
  flipH?: boolean
  flipV?: boolean
  /** source crop (a:srcRect) as fractions 0..1; null removes a:srcRect; undefined keeps */
...
export function patchImageParagraphXml(xml: string, patch: ImagePatch): string {
  ...
  if (patch.crop !== undefined) {
    out = out.replace(/<a:srcRect\b[^>]*\/>/g, '')
    if (patch.crop && (patch.crop.l || patch.crop.t || patch.crop.r || patch.crop.b)) {
      const l = Math.round(patch.crop.l * 100000)
      const t = Math.round(patch.crop.t * 100000)
      const r = Math.round(patch.crop.r * 100000)
      const b = Math.round(patch.crop.b * 100000)
      const srcRect = `<a:srcRect l="${l}" t="${t}" r="${r}" b="${b}"/>`
      if (/<a:blip\b[^>]*\/>/.test(out)) {
        out = out.replace(/(<a:blip\b[^>]*\/>)/, `$1${srcRect}`)
      } else if (/<\/a:blip>/.test(out)) {
        out = out.replace(/(<\/a:blip>)/, `$1${srcRect}`)
      }
    }
  }
...
function formatPPrChildren(format: ParaFormat | undefined): PPrChild[] {
  ...
  // autospace flags serialize only when explicitly off (Word's own on-state is
  // the omitted tag; writing val="1" would needlessly dirty every paragraph)
  if (format.autoSpaceDE === false)
    out.push({ name: 'w:autoSpaceDE', xml: '<w:autoSpaceDE w:val="0"/>' })
  if (format.autoSpaceDN === false)
    out.push({ name: 'w:autoSpaceDN', xml: '<w:autoSpaceDN w:val="0"/>' })
...
export function runFragmentXml(run: Run, insideLink: boolean = false): string {
...
const RUN_MANAGED_GROUPS: Array<{ key: string; tags: string[] }> = [
  ...
  { key: 'charSpacing', tags: ['w:spacing'] },
  { key: 'charScale', tags: ['w:w'] },
...
function revisionRPrChangeXml(run: Run): string | null {
  ...
  if (old.underline) {
    const val = typeof old.underline === 'string' ? old.underline : 'single'
    const colAttr = old.underlineColor ? ` w:color="${escapeXmlAttr(old.underlineColor)}"` : ''
    props.push(`<w:u w:val="${escapeXmlAttr(val)}"${colAttr}/>`)
  }
...
function modelRPrChildren(run: Run, insideLink: boolean): PPrChild[] {
  ...
  if (run.charSpacingTwips) {
    out.push({ name: 'w:spacing', xml: `<w:spacing w:val="${run.charSpacingTwips}"/>` })
  }
  if (run.charScalePct) {
    out.push({ name: 'w:w', xml: `<w:w w:val="${run.charScalePct}"/>` })
  }
  ...
  if (run.underline) {
    const val = typeof run.underline === 'string' ? run.underline : 'single'
    const colAttr = run.underlineColor ? ` w:color="${escapeXmlAttr(run.underlineColor)}"` : ''
    out.push({ name: 'w:u', xml: `<w:u w:val="${escapeXmlAttr(val)}"${colAttr}/>` })
  }
  if (run.shading) {
    out.push({
      name: 'w:shd',
...
export function mergeRPrModel(rawRPr: string, run: Run, insideLink: boolean): string {
  ...
  const groupEqual = (key: string): boolean => {
    switch (key) {
      ...
      case 'charSpacing': {
        const raw = rawAttr(rawOf('w:spacing'), 'w:val')
        return (raw ? parseInt(raw, 10) || undefined : undefined) === run.charSpacingTwips
      }
      case 'charScale': {
        const raw = rawAttr(rawOf('w:w'), 'w:val')
        return (raw ? parseInt(raw, 10) || undefined : undefined) === run.charScalePct
      }
      case 'size': {
      ...
      case 'underline': {
        const uNode = rawOf('w:u')
        const val = rawAttr(uNode, 'w:val')
        const color = rawAttr(uNode, 'w:color')
        const runVal =
          typeof run.underline === 'string' ? run.underline : run.underline ? 'single' : undefined
        if (runVal === undefined) return val === undefined || val === 'none'
        const expectedVal = val ?? 'single'
        const valMatches = runVal === expectedVal
        const colorMatches = (run.underlineColor ?? undefined) === (color ?? undefined)
        return valMatches && colorMatches
      }
      case 'vertAlign': {
...
export function generateRunXml(run: Run, insideLink: boolean = false): string {
