...
export interface ParaFormat {
  ...
  snapToGrid?: boolean
  /** CJK↔Latin letters auto spacing (w:autoSpaceDE; Word default on); false = explicitly off */
  autoSpaceDE?: boolean
  /** CJK↔digits auto spacing (w:autoSpaceDN; Word default on); false = explicitly off */
  autoSpaceDN?: boolean
...
export interface SectionSettings {
  ...
  colSpace?: number
  /** rule line between columns (w:cols w:sep); undefined = leave the tag untouched */
  colSep?: boolean
  /**
...
export interface HeaderFooter {
  ...
  paras?: HfParagraph[]
  /**
   * Style names of STYLEREF fields in part order; the Nth STYLEREF_MARK in
   * text/paras resolves against styleRefs[N] (renderer substitutes live
   * heading text, saving writes the field back).
   */
  styleRefs?: string[]
}
...
/**
 * Placeholder for STYLEREF fields in headers/footers; a private-use character
 * so literal text can never be mistaken for the field. The Nth marker of the
 * part maps to styleRefs[N]; the renderer substitutes the live heading text.
 */
export const STYLEREF_MARK = '\uE002'
...
export interface HfPartInfo {
  /** plain text, PAGE fields shown as PAGE_MARK, NUMPAGES as TOTAL_PAGES_MARK, STYLEREF as STYLEREF_MARK */
  text: string
  hasPageNumber: boolean
  paras: HfParagraph[]
  /** images in the part (display-only; text edits do not affect their bytes) */
  images?: HfImage[]
  /** style names of the part's STYLEREF fields, in marker order */
  styleRefs?: string[]
}
...
export interface CellBorders {
  ...
  bottom?: CellBorder
  right?: CellBorder
  /** Top-left to bottom-right diagonal border (w:tl2br) */
  tl2br?: CellBorder
  /** Top-right to bottom-left diagonal border (w:tr2bl) */
  tr2bl?: CellBorder
}
