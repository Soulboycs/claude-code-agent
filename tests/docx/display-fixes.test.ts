...
/**
 * Second review iteration (G round): display-layer defects found by the
 * independent sweep, each pinned so it cannot regress silently.
 */
describe('display-layer fixes from the second review iteration', () => {
  ...
  it('reviewer filter covers format revisions and escapes author names', () => {
    const marks = fs.readFileSync(
      'src/renderer/src/components/word/editor/marks.ts',
      'utf8',
    )
